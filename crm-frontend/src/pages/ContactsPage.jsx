    ...
    return (
        <div className="animate-fade-in pb-12">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
                <div>
                    <h1 className="text-[40px] font-bold text-apple-text tracking-tight mb-2">Contacts</h1>
                    <p className="text-[17px] text-apple-gray">
                        <span className="font-semibold text-apple-text">{contacts.length}</span> people in your network
                    </p>
                </div>
            ...
            {loading ? (
                <div className="bg-white rounded-[24px] shadow-apple-sm p-16 flex flex-col items-center justify-center space-y-4">
                    ...
                    <p className="text-apple-gray text-[15px] font-medium">Loading contacts...</p>
                </div>
            ) : contacts.length === 0 ? (
                <div className="bg-white rounded-[24px] shadow-apple-sm p-16 text-center">
                    ...
                    <h3 className="text-[20px] font-semibold text-apple-text mb-2">No contacts found</h3>
                        ...
                        <table className="w-full text-left border-collapse">
                            ...
                            <tbody>
                                {paginatedContacts.map((c) => (
                                    <tr key={c.id} className="group hover:bg-apple-bg transition-colors cursor-pointer border-b border-apple-bg last:border-0">
                                        ...
                                        <td className="py-4 px-6">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <Link to={`/contacts/${c.id}`} className="font-medium text-[15px] text-apple-text hover:text-apple-blue transition-colors">
                                                    {c.name}
                                                </Link>
                                            </div>
                                        </td>
                                        <td className="py-4 px-6 text-[15px] text-apple-gray">{c.email}</td>
                                        <td className="py-4 px-6 text-[15px] text-apple-gray">{c.phone || '--'}</td>
                                        <td className="py-4 px-6">
                                            {c.company ? (
                                        ...
                                        <td className="py-4 px-6 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                ...
                                                <Link to={`/contacts/${c.id}`} className="inline-flex items-center justify-center w-8 h-8 rounded-full bg-apple-bg text-apple-gray hover:bg-apple-blue hover:text-white shadow-sm transition-all">
